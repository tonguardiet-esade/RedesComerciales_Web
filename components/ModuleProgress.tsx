  ...
  return (
    <div className="w-full max-w-2xl mx-auto mb-10 px-4">
      ...
      <div className="flex items-center justify-between relative">
        ...
        <div
          className="absolute left-0 top-1/2 transform -translate-y-1/2 h-1 bg-brand-secondary z-0 rounded-full transition-all duration-500"
        ...
        {steps.map((step) => {
          ...
          if (userLevel !== UserLevel.EJEMPLOS_DE_VENTA && step === 5) {
              label = "Test Final";
          }
            ...
            <div key={step} className="relative z-10 flex flex-col items-center">
              <div
                ...
                ${isActive 
                  ...
                  : isCompleted 
                    ? 'bg-brand-secondary text-white border-white shadow-sm' 
              ...
              <span className={`absolute top-12 text-[10px] font-bold whitespace-nowrap transition-colors duration-300 ${isActive ? 'text-brand-primary' : (isCompleted ? 'text-brand-secondary' : 'text-gray-300')}`}>
                {label}
              </span>
            </div>
          );
